...
from forms import LoginForm, MessageForm, UserEditForm, UserAddForm
...
def delete_like(msg_id):
    ...
    if msg not in g.user.likes:
        flash("Cannot perform requested action.", "danger")
        return redirect("/")
    
    existing_like = Likes.query.filter(Likes.user_id==g.user.id, Likes.message_id==msg_id).first()
    db.session.delete(existing_like)
    db.session.commit()

    flash(f"You un-liked {msg.user.username}'s post!", "success")
    return redirect('/')
...
def homepage():
    ...
    if g.user:
        followers_and_own_id = []
        followers_and_own_id.append(g.user.id)
        for _user in g.user.following:
            followers_and_own_id.append(_user.id)

        messages = (Message.query
                    .filter(Message.user_id.in_(followers_and_own_id))
                    .order_by(Message.timestamp.desc())
                    .limit(100))        
